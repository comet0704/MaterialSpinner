...
public class MaterialSpinner extends TextView {
    ...
    private boolean bWithIcon = false;
    ...
    private void setDrawableStart(int res) {
        if (hideArrow) {
            ...
            if (rtl) {
                ...
                if(res != 0) {
                    drawables[2] = getResources().getDrawable(res);
                }
            } else {
                if(res != 0) {
                    drawables[0] = getResources().getDrawable(res);
                }
                drawables[2] = arrowDrawable;
            }
            setCompoundDrawablesWithIntrinsicBounds(drawables[0], drawables[1], drawables[2], drawables[3]);
        }
        setCompoundDrawablePadding(popupdrwablePadding);
    }

    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.MaterialSpinner);
        int defaultColor = getTextColors().getDefaultColor();
        rtl = Utils.isRtl(context);

        int paddingLeft, paddingTop, paddingRight, paddingBottom;
        int defaultPaddingLeft, defaultPaddingTop, defaultPaddingRight, defaultPaddingBottom;
        final int defaultPopupPaddingLeft, defaultPopupPaddingTop, defaultPopupPaddingRight, defaultPopupPaddingBottom, defaultPopupDrawablePadding;
        ...
        defaultPopupDrawablePadding = resources.getDimensionPixelSize(R.dimen.ms__popup_drwable_padding);
        ...
        try {
            ...
            popupdrwablePadding =
                    ta.getDimensionPixelSize(R.styleable.MaterialSpinner_ms_popup_drawable_padding, defaultPopupDrawablePadding);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if (item instanceof ItemWithIcon) {
                    setText(((ItemWithIcon) item).text);
                    setDrawableStart(((ItemWithIcon) item).iconRes);
                } else {
        ...
        if (savedState instanceof Bundle) {
            ...
            if (adapter != null) {
                if (nothingSelected && !TextUtils.isEmpty(hintText)) {
                    ...
                    setTextColor(textColor);
                    if (adapter.get(selectedIndex) instanceof ItemWithIcon) {
                        setText(((ItemWithIcon) adapter.get(selectedIndex)).text);
                        setDrawableStart(((ItemWithIcon) adapter.get(selectedIndex)).iconRes);
                    } else {
    ...
    public void setSelectedIndex(int position) {
        if (adapter != null) {
            if (position >= 0 && position <= adapter.getCount()) {
                ...
                selectedIndex = position;
                if (adapter.get(selectedIndex) instanceof ItemWithIcon) {
                    setText(((ItemWithIcon) adapter.get(position)).text);
                    setDrawableStart(((ItemWithIcon) adapter.get(selectedIndex)).iconRes);
                } else {
    ...
    public void setOnItemWithIconSelectedListener(@Nullable OnItemSelectedListener onItemSelectedListener) {
        this.onItemSelectedListener = onItemSelectedListener;
    }
    ...
    public <T> void setItemsWithIcon(@NonNull T... items) {
        setItems(Arrays.asList(items));
    }
    ...
    public <T> void setItems(@NonNull List<T> items) {
        adapter = new MaterialSpinnerAdapter<>(getContext(), items)
                ...
                .setPopupDrwablePadding(popupdrwablePadding)
    ...
    public static class ItemWithIcon {
        public String text = "";
        public int iconRes = 0;

        public ItemWithIcon(String p_text, int p_iconRes) {
            text = p_text;
            iconRes = p_iconRes;
        }
    }
